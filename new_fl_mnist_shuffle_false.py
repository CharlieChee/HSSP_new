...
def federated_datasets(num_clients=10, batch_size=64):
    # 定义针对 MNIST 数据集的转换：将图片转换为张量并进行归一化
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))  # 由于是灰度图，只需要一个通道的平均值和标准差
    ])
    # 加载 MNIST 训练和测试数据集
...
def federated_train_old(num_epochs, client_trainloaders, test_loader, global_model, device,full,save_batch_idx):
    ...
    log_file = Path('./expdata/acc/new_fl_mnist_shuffle_false.log')
        ...
        for client_idx, train_loader in enumerate(client_trainloaders):
            ...
            for batch_idx,(data, target) in enumerate(train_loader):
                ...
                if batch_idx == save_batch_idx and client_idx == 0:
                    np.save(f'./expdata/truefl_mnist_false/images_epoch_{epoch}_batch_{save_batch_idx}.npy',
                            data.cpu().numpy())
                    np.save(f'./expdata/truefl_mnist_false/gradients_fc1_relu_binary_epoch_{epoch}_batch_{batch_idx}.npy',
...
if __name__ == "__main__":
    ...
    data, _ = next(iter(full))  # 使用full loader获取一批数据
    data = data.to(device)
    global_model(data)  # 执行前向传播
    initial_output_activation = global_model.fc1_relu_outputs[-1].detach().cpu().numpy()
    np.save(f'./expdata/truefl_mnist_false/initial_activation.npy', (initial_output_activation > 0).astype(int))
    print("Initial activation saved.")
