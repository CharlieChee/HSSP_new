...
def federated_datasets(num_clients=10, batch_size=64):
    ...
    for i in range(num_clients):
        ...
        # 筛选出类别为3的索引
        cat_indices = [idx for idx in client_indices if full_trainset.targets[idx] == 3]
        # 筛选出其他类别的索引
        other_indices = [idx for idx in client_indices if full_trainset.targets[idx] != 3]
...
def federated_train_old(num_epochs, client_trainloaders, test_loader, global_model, device,full,save_batch_idx):
    ...
    log_file = Path('./expdata/acc/new_fl.log')
        ...
        for client_idx, train_loader in enumerate(client_trainloaders):
            ...
            for batch_idx,(data, target) in enumerate(train_loader):
                ...
                if batch_idx == save_batch_idx and client_idx == 0:
                    np.save(f'./expdata/truefl_cat/images_epoch_{epoch}_batch_{save_batch_idx}.npy',
                            data.cpu().numpy())
                    np.save(f'./expdata/truefl_cat/gradients_fc1_relu_binary_epoch_{epoch}_batch_{batch_idx}.npy',
...
if __name__ == "__main__":
    ...
    full = create_full_train_loader(batchsize)
