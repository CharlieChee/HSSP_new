...
def federated_datasets(num_clients=10, batch_size=64):
    ...
    for i in range(num_clients):
        ...
        # 筛选出类别为8的索引
        cat_indices = [idx for idx in client_indices if full_trainset.targets[idx] == 8]
        # 筛选出其他类别的索引
        other_indices = [idx for idx in client_indices if full_trainset.targets[idx] != 8]

        # 随机打乱其他类别的索引
        shuffled_other_indices = torch.randperm(len(other_indices)).tolist()
        other_indices = [other_indices[i] for i in shuffled_other_indices]

        # 创建每个客户端的批次采样器
        def client_batch_sampler():
            # 首批数据固定为类别8
            yield cat_indices[:batch_size]
        ...
        client_trainloader = DataLoader(
            full_trainset,
            batch_sampler=client_batch_sampler(),
            num_workers=2
        )
        client_trainloaders.append(client_trainloader)

    testloader = DataLoader(full_testset, batch_size=batch_size, shuffle=False, num_workers=10)
...
def federated_train_old(num_epochs, client_trainloaders, test_loader, global_model, device,full,save_batch_idx):
    ...
    log_file = Path('./expdata/acc/new_fl_mnist_true.log')
        ...
        for client_idx, train_loader in enumerate(client_trainloaders):
            ...
            for batch_idx,(data, target) in enumerate(train_loader):
                ...
                if batch_idx == save_batch_idx and client_idx == 0:
                    np.save(f'./expdata/truefl_mnist_8/images_epoch_{epoch}_batch_{save_batch_idx}.npy',
                            data.cpu().numpy())
                    np.save(f'./expdata/truefl_mnist_8/gradients_fc1_relu_binary_epoch_{epoch}_batch_{batch_idx}.npy',
...
if __name__ == "__main__":
    ...
    client_trainloaders, test_loader = federated_datasets(num_clients, batch)
    full = create_full_train_loader(batch)
    federated_train_old(num_epochs, client_trainloaders, test_loader, global_model, device,full,save_batch_idx=0)
