...
def federated_datasets(num_clients=10, batch_size=64):
    ...
    for i in range(num_clients):
        ...
        trainloader = DataLoader(client_dataset, batch_size=batch_size, shuffle=True, num_workers=10)
        ...
        for client_idx, train_loader in enumerate(client_trainloaders):
            ...
            for batch_idx,(data, target) in enumerate(train_loader):
                ...
                if batch_idx == save_batch_idx and client_idx == 0:
                    np.save(f'./expdata/truefl/images_epoch_{epoch}_batch_{save_batch_idx}.npy',
                            data.cpu().numpy())
                    np.save(f'./expdata/truefl/gradients_fc1_relu_binary_epoch_{epoch}_batch_{batch_idx}.npy',
